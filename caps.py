#!/usr/bin/env python3
"""
caps.py  —  An iclock / ADMS push server for ZKTeco / eSSL devices that speak
the HTTP "push" protocol (User-Agent "iClock Proxy/1.09" and similar).

This is the full-featured server: everything door_open.py does, plus device
capability discovery (/caps) and parameter setting (/setopt, /setsensor).
...
------------
  ...
  * Lets you QUEUE commands (door open, parameter get/set, reboot) that fire
    on the device's next poll.
  * Harvests whatever option names the firmware actually supports, giving you
    its real capability surface.
...
---
    ...
    python3 caps.py                      # port 8081
    python3 caps.py 8081
...
?token=<value> or as an X-Auth-Token header:
    ...
    /caps?token=T                     queue capability queries
    /caps/show?token=T                show what came back
    /setopt?token=T&k=Name&v=Value    set a device parameter
    /raw?token=T&p=01010105           send an arbitrary control payload
    /status?token=T                   see what's queued

Commands are queued and run on the device's next poll (typically <15s).
...
_caps_seen = {}                 # {option_name: value} from GET OPTIONS replies

# Curated list of known ZK device option names, grouped into query batches.
# The device returns name=value for the ones it supports; unknowns are omitted.
# What comes back = this firmware's actual capability surface.
_CAP_QUERY_BATCHES = [
    # identity / versions
    ["~SerialNumber", "~DeviceName", "~ProductTime", "~OEMVendor",
     "MachineType", "~ZKFPVersion", "~Platform", "FirmVer", "PushVersion",
     "~ExtendFmt", "~ExtendOPLogFmt"],
    # capacity / counts
    ["LockCount", "AuxOutCount", "AuxInCount", "ReaderCount", "DoorCount",
     "UserCount", "FPCount", "FaceCount", "~MaxUserCount", "~MaxFingerCount",
     "~MaxFaceCount", "TransactionCount"],
    # door / lock / access control
    ["Door1Drivertime", "Door2Drivertime", "Door1SensorType",
     "Door1Detectortime", "Door1CloseAndLock", "Door1KeepOpenTimeZone",
     "Door1SupperPassWord", "Door1ForcePassWord", "AntiPassback",
     "InterLock", "Door1Intertime"],
    # verification / display / voice
    ["VOICE", "Volume", "~VerifyStyle", "ShowScore", "MSpeed",
     "DisplayUserInfo", "OnlyPINCard", "FaceThreshold", "FaceFunOn",
     "FingerFunOn", "GreetingText", "CustomVoice", "VerifyStyles"],
    # comms / push behavior
    ["Realtime", "Delay", "TransFlag", "TransTimes", "TransInterval",
     "TimeZone", "Encrypt", "ServerVer", "ADMSVer", "PushProtVer",
     "PushOptionsFlag", "ErrorDelay"],
    # attendance / workcode / misc
    ["WorkCode", "~PIN2Width", "~IsOnlyRFMachine", "~UserExtFmt",
     "CameraOn", "PhotoFunOn", "UserPicURLFunOn", "~MaxAttLogCount",
     "AttPhotoCount", "IsSupportABTMF", "CompatOldFirmware"],
]

# Option names whose values are device credentials — never print or store them.
_SECRET_OPTIONS = {"Door1SupperPassWord", "Door1ForcePassWord",
                   "Door2SupperPassWord", "Door2ForcePassWord"}
...
def _harvest_caps(body):
    """
    Pull name=value pairs out of a GET OPTIONS reply and store them.
    Replies may be comma-separated, newline-separated, or &-separated, e.g.
      ~SerialNumber=<serial>,LockCount=1,AuxOutCount=0
      GreetingText=Thank you
    We skip the command-ack fields (ID/Return/CMD) and redact any option that
    carries a device password.
    """
    skip = {"ID", "Return", "CMD"}
    # normalize separators to newlines, then split each line on the FIRST '='
    ...
    for line in normalized.splitlines():
        ...
        if not name or name in skip:
            continue
        _caps_seen[name] = "<redacted>" if name in _SECRET_OPTIONS \
            else value.strip()
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if path == "/setsensor":
            # Set Door1SensorType.  0 = no sensor, 1 = normally-open sensor,
            # 2 = normally-closed sensor. If a door sensor is gating remote
            # door-open, try 0 to rule it out, then retry /open.
            val = q.get("v", ["0"])[0]
            target = self._target()
            if not target:
                return
            _cmd_id["n"] += 1
            cmd = f"C:{_cmd_id['n']}:SET OPTIONS Door1SensorType={val}"
            self._enqueue(target, cmd, f"SET Door1SensorType={val}")
            return self._reply(
                f"Queued: {cmd}\n"
                f"Sets door sensor type to {val} (0=none, 1=NO, 2=NC).\n"
                "After it fires (watch for Return=0), retry /open and /hold.\n"
                "Re-run /caps to confirm the new value stuck.\n")

        if path == "/setopt":
            # Generic parameter setter, e.g. /setopt?k=Door1Drivertime&v=8
            # or /setopt?k=Door1KeepOpenTimeZone&v=1
            key = q.get("k", [""])[0]
            val = q.get("v", [""])[0]
            if not key:
                return self._reply("Usage: /setopt?k=<ParamName>&v=<value>\n")
            target = self._target()
            if not target:
                return
            _cmd_id["n"] += 1
            cmd = f"C:{_cmd_id['n']}:SET OPTIONS {key}={val}"
            self._enqueue(target, cmd, f"SET {key}={val}")
            return self._reply(
                f"Queued: {cmd}\n"
                "Watch for Return=0, then /caps to confirm it stuck.\n")

        if path == "/caps":
            # Dump device capabilities by querying a big curated list of known
            # ZK option names, in batches. Whatever the device returns is what
            # this firmware actually exposes. Answers arrive via devicecmd
            # (or cdata) POSTs and are collected in _caps_seen.
            target = self._target()
            if not target:
                return
            for names in _CAP_QUERY_BATCHES:
                _cmd_id["n"] += 1
                _queue.setdefault(target, []).append(
                    f"C:{_cmd_id['n']}:GET OPTIONS {','.join(names)}")
            print(f"[{_stamp()}] QUEUED {len(_CAP_QUERY_BATCHES)} capability-"
                  f"query batches for {target}")
            return self._reply(
                f"Queued {len(_CAP_QUERY_BATCHES)} capability queries. They "
                "fire one per poll (~10-15s each), so give it a minute.\n"
                "Watch the log for the replies, then open  /caps/show  to see "
                "the collected results.\n")

        if path == "/caps/show":
            if not _caps_seen:
                return self._reply(
                    "Nothing collected yet. Run /caps first, wait ~1-2 min "
                    "for the batches to fire, then reload this.\n")
            lines = [f"Device capabilities reported by {_known_serial['sn']}:",
                     "=" * 50]
            for k in sorted(_caps_seen):
                lines.append(f"  {k} = {_caps_seen[k]}")
            lines.append("")
            lines.append(f"({len(_caps_seen)} parameters returned)")
            return self._reply("\n".join(lines) + "\n")
...
if __name__ == "__main__":
    ...
    print("=" * 70)
    print(" ZKTeco / eSSL push server  (iclock / ADMS)  —  full/caps build")
    print("=" * 70)
    ...
    if not AUTH_TOKEN:
        ...
        print("    export ZK_AUTH_TOKEN=\"$(python3 -c "
              "'import secrets;print(secrets.token_urlsafe(24))')\"")
        print()
    print(" Control endpoints (same network, token required):")
    print(f"     http://{ip}:{PORT}/open{tok}")
    print(f"     http://{ip}:{PORT}/caps{tok}   then  /caps/show{tok}")
